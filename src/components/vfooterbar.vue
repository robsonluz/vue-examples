<template>
  <div class="has-background-grey-dark mt-6">

    <div class="container">
      <footer class="footer has-background-grey-dark has-text-white">

        <div class="content has-text-centered">
          <p>
            Criado por
            Alunos 4º período do curso de Negócios Digitais da
            <a href="https://fae.edu" target="_blank">FAE Centro Universitário</a>
          </p>
        </div>

        <div class="content has-text-centered">
          <a href="https://www.facebook.com" target="_blank" class="is-size-2">
              <i class="fab fa-facebook"></i>
          </a>
          &nbsp;
          <a href="https://www.instagram.com" target="_blank" class="is-size-2">
              <i class="fab fa-instagram"></i>
          </a>  
        </div>          

      </footer>
    </div>
  </div>
</template>